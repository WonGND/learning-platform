...
> ⚠ **주의**: 멤버십 게이트는 **보안 경계가 아니라 UX 장치**다. 입장 코드는 클라이언트(브라우저)에서
> 검증되고 번들·공개 소스에 그대로 노출된다. "무료 멤버십 채널 유도" 용도로만 쓰고,
> **유료 콘텐츠를 이 코드로 보호하지 마라.** (콘텐츠 자체 판매 시에는 인증·결제 백엔드가 필수)
...
## 보안 모델 & 개인정보 (2026-07 감사 반영)

**수익 모델: A (리드젠)** — 무료 콘텐츠로 신뢰를 쌓아 외부 유료 강의/부트캠프로 유도한다.
콘텐츠 자체를 판매하지 않으므로 인증·결제 백엔드는 두지 않는다.

| 항목 | 상태 |
|------|------|
| 멤버십 게이트 | **UX 장치이며 보안 경계가 아님** — 입장 코드·유료 챕터 전문이 번들과 공개 레포에 노출됨 (모델 A에서는 의도된 설계) |
| XSS 방어 | `marked` 출력에 DOMPurify sanitize 적용 (`src/components/Markdown.tsx`) |
| CSP | 프로덕션 빌드 시 meta 태그 주입 (`vite.config.ts`) — 외부 origin 스크립트/폰트 차단 |
...
- 개인정보 보호법(PIPA): 수집 항목·목적·보유 기간 고지와 **동의** 획득, 처리방침 페이지 게시
- GDPR 대상 사용자가 있다면: 처리 법적 근거 명시, 열람·삭제권 대응 절차
- 분석은 **최소·동의 기반**으로: PII 없이 퍼널 단계 이벤트만, 동의 전 수집 금지
